...
class RGBDataset(Dataset):
    ...
    def get_data_dir(self,root_dir):
        ...
        for file1 in dirs1:
            ...
            for file2 in dirs2:
                ...
                self.rgb_file_path.append(rgb_path)
                ...
                self.depth_file_path.append(depth_path)

                gt_path = os.path.join(file_path[0],file2)
                gt_path = os.path.join(gt_path,"*gt.png")
                gt_path = glob.glob(gt_path)
                self.gt_file_path.append(gt_path)

        
    def get_data(self):
            self.get_data_dir(self.root_dir)
            rgb_file = []
            depth_file = []
            gt_file = []
            for file in self.rgb_file_path:
    ...
    def __getitem__(self,idx):
        ...
        gt = self.gt_file[idx]
        GT = gt
        target_path = self.depth_file_path[idx][0][:-32]
        if self.train:
            gt = self.minmax(gt)
        ...
        sample = {'guidance': image, 'target': target, 'gt': gt,'depth_min':depth_min,'depth_max':depth_max,'target_path':target_path,'GT':GT}
