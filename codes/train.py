...
parser.add_argument('--parameter',  default='./parameter/', help='name of parameter file')
...
def validate(net, root_dir="/data/yonghui/RGB-D-D"):

    data_transform = transforms.Compose([
        transforms.ToTensor()
    ])
   #test_dataset = NYU_v2_datset(root_dir="/data1/yonghui_data/NYU v2", transform=data_transform, train=False)
   ...
   # test_minmax = np.load('%s/test_minmax.npy',%root_dir)
    ...
    for idx, data in enumerate(t):
        ...
        minn ,maxx= data['depth_min'].numpy()[0],data['depth_max'].numpy()[0]
        
        out = net((guidance, target))
        rmse[idx] = calc_rmse(gt[0,0].cpu().numpy(), out[0,0].cpu().numpy(),minn,maxx)
...
#nyu_dataset = NYU_v2_datset(root_dir='/data1/yonghui_data/NYU v2', transform=data_transform)
